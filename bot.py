...
async def select_restaurant(callback_query: types.CallbackQuery):
    ...
    # Check the status of each restaurant
    import pytz
    from datetime import datetime
    ...
    for rest_id, rest_name, weekdays_schedule, weekend_schedule, closed_days in restaurants:
        ...
        if closed_days and current_day in closed_days.split(","):
            is_open = False  # Closed on this day
...
async def select_dishes(callback_query: types.CallbackQuery):
    ...
    if is_open:
        ...
        # Restaurant is closed, display the menu but prevent adding dishes to cart
        keyboard = InlineKeyboardMarkup(row_width=1)
        for dish_id, dish_name in dishes:
            # Add a message that dishes cannot be added to the cart because the restaurant is closed
            keyboard.add(
                InlineKeyboardButton(f"{dish_name} (⏰ не добавляется в корзину)",
                                     callback_data=f"dish_{dish_id}_{restaurant_id}_{category_id}")
            )
        keyboard.add(InlineKeyboardButton("🔙 Назад", callback_data=f"category_{category_id}"))

        await callback_query.message.edit_text(
            f"Ресторан {restaurant_name} сейчас закрыт. Вы можете просматривать меню, но не можете добавлять блюда в "
            f"корзину.",
            reply_markup=keyboard
        )
